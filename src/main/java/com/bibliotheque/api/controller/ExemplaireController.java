...
public class ExemplaireController {
    ...
    public ResponseEntity<List<Exemplaire>> getAllExemplaires() {
        ...
        return ResponseEntity.ok(exemplaires);    // HTTP 200 + liste des exemplaires
    }
    ...
    public ResponseEntity<Exemplaire> createExemplaire(@RequestBody Exemplaire exemplaire) {
        Exemplaire nouvelExemplaire = exemplaireService.createExemplaire(exemplaire);
        return ResponseEntity.status(HttpStatus.CREATED).body(nouvelExemplaire);  // HTTP 201
    }
    ...
    // GET /api/exemplaires/search?etat=neuf → Rechercher par état
    @GetMapping("/search")
    public ResponseEntity<List<Exemplaire>> findByEtat(@RequestParam Exemplaire.Etat etat) {
        List<Exemplaire> exemplaires = exemplaireService.filterExemplairesByEtat(etat);
